...
def Prepare_DataLoaders(opt, split, input_size=(224,224)):
    ...
    if opt.five_crop:
        train_data_transforms_list.insert(3,transforms.FiveCrop(input_size))
        train_data_transforms_list.insert(4,transforms.Lambda(lambda crops: [transforms.ToTensor()(crop) for crop in crops]))
        train_data_transforms_list.insert(5,transforms.Lambda(lambda crops: [normalize(crop) for crop in crops]))
        train_data_transforms_list.insert(6,transforms.Lambda(lambda crops: torch.stack(crops)))
    else:
        if opt.center_crop:
            train_data_transforms_list.insert(3, transforms.CenterCrop(input_size))
        else:
            train_data_transforms_list.insert(3, transforms.RandomResizedCrop(input_size, scale=(.8,1.0)))
        train_data_transforms_list.insert(4, transforms.ToTensor())
        train_data_transforms_list.insert(5, transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]))

    if opt.rotation_need:
        train_data_transforms_list.insert(4, transforms.RandomAffine(opt.degree))
